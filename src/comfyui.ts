...
export async function generateImage(
  ...
  try {
    ...
    (workflow as any)[6].inputs.text = prompt;
    ...
    const viewResponse = await fetch(
      `${serverUrl}/view?filename=${filename}&subfolder=&type=output`
    );
    ...
    const imagePath = Uri.file(`comfyui_generated_${timestamp}.png`);
    const workspaceFolder = vscode.workspace.workspaceFolders?.[0];
    if (!workspaceFolder) {
      throw new Error('No workspace folder open.');
    }
    const fullPath = Uri.joinPath(workspaceFolder.uri, imagePath.path);
