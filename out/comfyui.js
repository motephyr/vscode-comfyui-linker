...
async function generateImage(prompt, config) {
    try {
        ...
        const workflowTemplateStr = config.get('workflowTemplate', '');
        console.log('Retrieved workflowTemplate:', workflowTemplateStr);
        const promptNodeId = config.get('promptNodeId', '6');
        const promptInputKey = config.get('promptInputKey', 'text');
        // Validate server URL
        ...
        for (const [outputNodeId, outputObj] of Object.entries(outputs)) {
            if (typeof outputObj !== 'object' || outputObj === null)
                continue;
            if (!('images' in outputObj) || !Array.isArray(outputObj.images) || outputObj.images.length === 0)
                continue;
            ...
            for (let i = 0; i < outputObj.images.length; i++) {
                ...
                const viewResponse = await fetch(`${serverUrl}/view?filename=${filename}&subfolder=${subfolder}&type=${type}`);
                if (!viewResponse.ok) {
                ...
                const fullPath = vscode_1.Uri.joinPath(workspaceFolder.uri, imageFilename);
